...
def decode():
    with tf.Session() as sess:
        sess, model, enc_vocab, rev_dec_vocab = init_session(sess, conf='seq2seq.ini')
        model.batch_size = 1
        ...
        while sentence:
            res = list()
            for fir in sentence.split("，"):
                res.append(fir)
            res_set = set(res)
            # remove some duplicated lyrics
            for i in range(10):
                n = 4
                while len(data_utils.basic_tokenizer("，".join(res[-n:]))) > 65 and n > 1:
                    n -= 1
                cand = decode_line(sess, model, enc_vocab, rev_dec_vocab, u'，'.join(res[-n:]))
