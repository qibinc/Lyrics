...
class Model(object):
    ...
    def __init__(self,config=None,opts=None):
        ...
        # by their names
        #######################################################################
        self.params_vars = ['self.params_np']
        self.learn_vars  = ['self.lr','self.lr_decay','self.beta','self.seed',
                'self.min_impr','self.llogp', 'self.debug','self.valid_logp',
                'self.lr_divide']
        self.mode_vars   = ['self.mode','self.obj','self.gamma','self.batch']
        self.data_vars   = ['self.domain', 'self.wvecfile', 'self.modelfile',
                'self.vocabfile', 'self.trainfile','self.validfile','self.testfile',
                'self.percentage']
        self.gen_vars    = ['self.topk','self.beamwidth','self.overgen',
                'self.detectpairs','self.verbose']
        self.model_vars  = ['self.gentype','self.di','self.dh']
    ...
    def testNet(self):
        ...
        if refcnts[0] != 0:
            ...
            print('refcnts[0] == 0 for some reason')
        print ('----------------------------------------------')
        ...
        if gencnts[0] != 0:
            ...
            print('gencnts[0] == 0 for some reason')
    ...
    def saveNet(self):
        ...
        self.updateNumpyParams()
        learn_key = [i for i in self.learn_vars]
        learn_val = []
        for i in self.learn_vars:
            learn_val.append(eval(i))
        learn_dict = dict([(i, j) for i, j in zip(learn_key, learn_val)])

        data_key = [i for i in self.data_vars]
        data_val = []
        for i in self.data_vars:
            data_val.append(eval(i))
        data_dict = dict([(i, j) for i, j in zip(data_key, data_val)])

        gen_key = [i for i in self.gen_vars]
        gen_val = []
        for i in self.gen_vars:
            gen_val.append(eval(i))
        gen_dict = dict([(i, j) for i, j in zip(gen_key, gen_val)])

        model_key = [i for i in self.model_vars]
        model_val = []
        for i in self.model_vars:
            model_val.append(eval(i))
        model_dict = dict([(i, j) for i, j in zip(model_key, model_val)])

        mode_key = [i for i in self.mode_vars]
        mode_val = []
        for i in self.mode_vars:
            mode_val.append(eval(i))
        mode_dict = dict([(i, j) for i, j in zip(mode_key, mode_val)])

        params_key = [i for i in self.params_vars]
        params_val = []
        for i in self.params_vars:
            params_val.append(eval(i))
        params_dict = dict([(i, j) for i, j in zip(params_key, params_val)])
        ...
        bundle={
            ...
            'mode':mode_dict,
            'params':params_dict
            #'learn' :dict( [(name,eval(name)) for name in aa]  ),
